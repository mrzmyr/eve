...
export interface AddConnectionsOptions {
  ...
  prompter: Prompter;
  /** Runs after connector selection but before the connection file is written. */
    ...
    async perform({ state }): Promise<ProjectResolution> {
      ...
      for (const plan of state.connectionSelection) {
        ...
        switch (plan.provision.kind) {
          case "connect": {
            const canonicalConnectorUid = canonicalConnectorUidForEntry(plan.entry);
            if (canonicalConnectorUid === undefined) {
              throw new Error(`Connection ${plan.slug} has no canonical connector UID.`);
            }
        ...
        try {
          await options.beforeScaffold?.(projectRoot);
